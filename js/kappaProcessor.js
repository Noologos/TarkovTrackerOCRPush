...
import { updateCollapsibleVisibility, showGlobalSpinner } from './ui.js'; // Added showGlobalSpinner
...
export async function processKappaImage() {
    ...
    showKappaSpinner(true); // Show Kappa sectional spinner
    if (!wasGlobalSpinnerActive) { // If global wasn't active, Process All didn't trigger this
        UI.setButtonEnabled(DOM.processAllButton, false); // Disable process all if kappa started manually
    }
    updateButtonStates(); // This will disable the kappa process button
    ...
    if (!mainCtx) {
        ...
        if (wasGlobalSpinnerActive) showGlobalSpinner(false); // Hide global if it was on
    ...
    workerInstance.onmessage = (event) => {
        ...
        switch (type) {
            ...
            case 'result':
                ...
                showKappaSpinner(false); // Hide Kappa sectional spinner
                if (wasGlobalSpinnerActive) showGlobalSpinner(false); // Hide global spinner if it was active
